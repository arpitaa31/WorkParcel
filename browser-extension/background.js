...
let port = null;
let state = { status: "APP NOT RUNNING", browser: detectBrowser(), extensionVersion: "0.1.0", lastConnected: null, windowCount: 0, tabCount: 0 };
...
async function collectSnapshot() {
  const captured = new Date().toISOString(); const tabs = await chrome.tabs.query({}); const visibleTabs = tabs.filter(tab => !tab.incognito && tab.id !== undefined && tab.id !== null).sort((a, b) => Number(a.windowId) - Number(b.windowId) || Number(a.index) - Number(b.index) || Number(a.id) - Number(b.id)); const windowIds = [...new Set(visibleTabs.map(tab => tab.windowId))].sort((a, b) => Number(a) - Number(b)); const windowKeys = new Map(windowIds.map((id, index) => [id, `window-${index}`])); const windows = new Set(windowIds.map(id => String(id))); const output = []; const windowDetails = new Map();
  if (chrome.windows?.getAll) { try { for (const window of await chrome.windows.getAll({ populate: false })) if (window?.id !== undefined && window?.id !== null) windowDetails.set(window.id, window); } catch { } }
  for (const tab of visibleTabs) {
    ...
    const window = windowDetails.get(tab.windowId) || {}; windows.add(String(tab.windowId)); output.push({ sessionTabId: String(tab.id), sessionWindowId: String(tab.windowId), windowGroupKey: windowKeys.get(tab.windowId) || "window-0", sessionGroupId: hasGroup ? String(tab.groupId) : null, url, title: tab.title || domain || "Untitled tab", domain, browser: state.browser, connectionId: state.connectionId || null, tabIndex: tab.index, pinned: !!tab.pinned, active: !!tab.active, groupTitle: group?.title || null, groupColor: group?.color || null, faviconUrl: safeFavicon(tab.favIconUrl), incognito: !!tab.incognito, canRestore: supported, unsupportedReason: supported ? null : "Browser-internal or unsupported URL", capturedAtUtc: captured, windowLeft: Number.isInteger(window.left) ? window.left : null, windowTop: Number.isInteger(window.top) ? window.top : null, windowWidth: Number.isInteger(window.width) ? window.width : null, windowHeight: Number.isInteger(window.height) ? window.height : null, windowState: typeof window.state === "string" ? window.state : null, windowFocused: !!window.focused });
  }
...
async function openTabs(savedTabs) {
  ...
  for (const [windowKey, windowItems] of groupedItems) {
    ...
    if (first && !windows.has(windowKey)) {
      ...
      else { try { const createOptions = { focused: !!first.windowFocused, url: first.url }; if (Number.isInteger(first.windowLeft)) createOptions.left = first.windowLeft; if (Number.isInteger(first.windowTop)) createOptions.top = first.windowTop; if (Number.isInteger(first.windowWidth)) createOptions.width = first.windowWidth; if (Number.isInteger(first.windowHeight)) createOptions.height = first.windowHeight; if (["normal", "minimized", "maximized", "fullscreen"].includes(first.windowState)) createOptions.state = first.windowState; const createdWindow = await chrome.windows.create(createOptions); if (createdWindow.id !== undefined && createdWindow.id !== null) windows.set(windowKey, createdWindow.id); } catch { } }
    }
  ...
  if (chrome.windows?.update) for (const [windowKey, windowId] of windows) { const first = groupedItems.get(windowKey)?.find(item => item && isRestorable(item.url)); if (!first) continue; const updateOptions = {}; if (Number.isInteger(first.windowLeft)) updateOptions.left = first.windowLeft; if (Number.isInteger(first.windowTop)) updateOptions.top = first.windowTop; if (Number.isInteger(first.windowWidth)) updateOptions.width = first.windowWidth; if (Number.isInteger(first.windowHeight)) updateOptions.height = first.windowHeight; if (["normal", "minimized", "maximized", "fullscreen"].includes(first.windowState)) updateOptions.state = first.windowState; if (first.windowFocused) updateOptions.focused = true; if (Object.keys(updateOptions).length) try { await chrome.windows.update(windowId, updateOptions); } catch { } }
