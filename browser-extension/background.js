...
let port = null;
let state = { status: "APP NOT RUNNING", browser: detectBrowser(), extensionVersion: "0.2.0", lastConnected: null, windowCount: 0, tabCount: 0 };
...
async function openTabs(savedTabs) {
  ...
  for (const [windowKey, windowItems] of groupedItems) {
    ...
    if (first && !windows.has(windowKey)) {
      ...
      else { try { const createdWindow = await chrome.windows.create({ url: first.url }); if (createdWindow.id !== undefined && createdWindow.id !== null) windows.set(windowKey, createdWindow.id); } catch { } }
    }
