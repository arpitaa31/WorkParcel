...
BrowserFaviconUrl=$browserFaviconUrl, BrowserCapturedAt=$browserCapturedAt, BrowserLastOpenedAt=$browserLastOpenedAt WHERE Id=$id;";
    ...
    public async Task UpdateItemStatesAsync(IEnumerable<ParcelItem> items, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var item in items)
        {
            ...
            command.Parameters.AddWithValue("$updated", DbValue.Time(item.UpdatedAt));
            command.Parameters.AddWithValue("$verified", DbValue.Db(DbValue.Time(item.LastVerifiedAt)));
    ...
    public async Task<bool> DeleteItemWithHistoryAsync(Guid itemId, ParcelHistoryEntry history, CancellationToken cancellationToken = default)
    {
        ...
        command.Parameters.AddWithValue("$parcel", history.ParcelId.ToString());
    ...
    public async Task ReplaceItemsWithHistoryAsync(Parcel parcel, IReadOnlyList<ParcelItem> items, ParcelHistoryEntry history, CancellationToken cancellationToken = default)
    {
        ...
        await using (var delete = connection.CreateCommand())
        {
            ...
            delete.CommandText = "DELETE FROM ParcelItems WHERE ParcelId=$parcel;";
            delete.Parameters.AddWithValue("$parcel", parcel.Id.ToString());
        ...
        await using (var update = connection.CreateCommand())
        {
            ...
            update.CommandText = "UPDATE Parcels SET State=$state, UpdatedAt=$updated, LastOpenedAt=$opened, LastPackedAt=$packed WHERE Id=$id;";
            update.Parameters.AddWithValue("$state", parcel.Status.ToString());
            update.Parameters.AddWithValue("$updated", DbValue.Time(parcel.UpdatedAt));
            update.Parameters.AddWithValue("$opened", DbValue.Db(DbValue.Time(parcel.LastOpenedAt)));
            update.Parameters.AddWithValue("$packed", DbValue.Db(DbValue.Time(parcel.LastPackedAt)));
            update.Parameters.AddWithValue("$id", parcel.Id.ToString());
...
VALUES($id,$parcel,$type,$name,$value,$identity,$detail,$created,$updated,$verified,$sort,$missing,$inaccessible,$changed,$executable,$arguments,$working,$windowTitle,$windowClassName,$process,$aumid,$fileSize,$fileModified,$fingerprint,$icon,$note,$launchEnabled,$closeSupported,$browserFamily,$browserDomain,$browserWindowGroupId,$browserTabIndex,$browserPinned,$browserActive,$browserTabGroupId,$browserTabGroupTitle,$browserTabGroupColor,$browserFaviconUrl,$browserCapturedAt,$browserLastOpenedAt);";
    ...
    private static async Task TouchParcelAsync(SqliteConnection connection, SqliteTransaction transaction, Guid parcelId, DateTime timestamp, CancellationToken cancellationToken)
    {
        await using var command = connection.CreateCommand();
        command.Transaction = transaction;
        command.CommandText = "UPDATE Parcels SET UpdatedAt=$updated WHERE Id=$id;";
        command.Parameters.AddWithValue("$updated", DbValue.Time(timestamp));
        command.Parameters.AddWithValue("$id", parcelId.ToString());
        await command.ExecuteNonQueryAsync(cancellationToken);
    }
    ...
    private static void AddItemParameters(SqliteCommand command, ParcelItem item, bool includeCreated = false)
    {
        ...
        command.Parameters.AddWithValue("$identity", DbValue.Db(item.NormalizedIdentity));
        command.Parameters.AddWithValue("$detail", DbValue.Db(item.SecondaryDetail));
        ...
        command.Parameters.AddWithValue("$inaccessible", item.IsInaccessible ? 1 : 0);
        command.Parameters.AddWithValue("$changed", item.HasChanged ? 1 : 0);
        command.Parameters.AddWithValue("$executable", DbValue.Db(item.ExecutablePath));
        command.Parameters.AddWithValue("$arguments", DbValue.Db(item.LaunchArguments));
        command.Parameters.AddWithValue("$working", DbValue.Db(item.WorkingDirectory));
        command.Parameters.AddWithValue("$windowTitle", DbValue.Db(item.WindowTitle));
        command.Parameters.AddWithValue("$windowClassName", DbValue.Db(item.WindowClassName));
        command.Parameters.AddWithValue("$process", DbValue.Db(item.ProcessName));
        command.Parameters.AddWithValue("$aumid", DbValue.Db(item.ApplicationUserModelId));
        command.Parameters.AddWithValue("$fileSize", DbValue.Db(item.FileSize));
        command.Parameters.AddWithValue("$fileModified", DbValue.Db(DbValue.Time(item.FileModifiedAt)));
        command.Parameters.AddWithValue("$fingerprint", DbValue.Db(item.Fingerprint));
        command.Parameters.AddWithValue("$icon", DbValue.Db(item.IconCacheKey));
    ...
    private static ParcelHistoryEntry ReadHistory(SqliteDataReader reader) => new()
    {
        ...
        EventType = ParseHistoryEventType(reader["EventType"]),
    ...
    private static ParcelItem ReadItem(SqliteDataReader reader) => new()
    {
        Id = Guid.Parse(reader["Id"].ToString()!),
        ParcelId = Guid.Parse(reader["ParcelId"].ToString()!),
        ItemType = Enum.Parse<ParcelItemType>(reader["ItemType"].ToString()!, true),
        DisplayName = reader["DisplayName"].ToString()!,
        Value = reader["Value"].ToString()!,
        NormalizedIdentity = NullText(reader["NormalizedIdentity"]),
        SecondaryDetail = NullText(reader["SecondaryDetail"]),
        CreatedAt = DbValue.Time(reader["CreatedAt"]),
        UpdatedAt = DbValue.Time(reader["UpdatedAt"]),
        LastVerifiedAt = DbValue.NullableTime(reader["LastVerifiedAt"]),
        SortOrder = Convert.ToInt32(reader["SortOrder"]),
        IsMissing = Convert.ToInt32(reader["IsMissing"]) == 1,
        IsInaccessible = Convert.ToInt32(reader["IsInaccessible"]) == 1,
        HasChanged = Convert.ToInt32(reader["HasChanged"]) == 1,
        ExecutablePath = NullText(reader["ExecutablePath"]),
        LaunchArguments = NullText(reader["LaunchArguments"]),
        WorkingDirectory = NullText(reader["WorkingDirectory"]),
        WindowTitle = NullText(reader["WindowTitle"]),
        WindowClassName = NullText(reader["WindowClassName"]),
        ProcessName = NullText(reader["ProcessName"]),
        ApplicationUserModelId = NullText(reader["ApplicationUserModelId"]),
        FileSize = reader["FileSize"] is DBNull ? null : Convert.ToInt64(reader["FileSize"]),
        FileModifiedAt = DbValue.NullableTime(reader["FileModifiedAt"]),
        Fingerprint = NullText(reader["Fingerprint"]),
    ...
    private static ParcelHistoryEventType ParseHistoryEventType(object value) => Enum.TryParse<ParcelHistoryEventType>(value.ToString(), true, out var eventType) ? eventType : ParcelHistoryEventType.Updated;
